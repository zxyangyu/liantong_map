...
point_df = pd.read_json('result_json_' + sys.argv[1] + '.json').drop_duplicates()
detail_df = point_df[['imei', 'flowsize']]
detail_df.loc[:,'flowsize'] = detail_df['flowsize'] / 1024 / 1024 / 8
detail_df.columns = ['imei', '使用流量（MB）']
detail_df.to_excel('../html/FlowUsageDetails.xlsx',index=False)
...
for paths, zone_name, zone_adcode in zip(all_data_frame['matplotpath'], all_data_frame['zone_name'],
                                         all_data_frame['zone_adcode']):
    # print(zone_name)
    for path in paths:
...
agg_df1 = point_df.groupby(["city", "zone_name"]).agg(
    {"flowsize": np.sum, "ucc_count": np.sum, "zone_adcode": np.size}).reset_index()
agg_df2 = point_df.groupby(["city"]).agg(
    {"flowsize": np.sum, "ucc_count": np.sum, "zone_adcode": np.size}).reset_index()
...
agg_df = pd.concat([agg_df1, agg_df2, attach, attach.drop(columns='zone_name').drop_duplicates()]).drop_duplicates(['city','zone_name'], keep='first')
# agg_df.to_csv('static/data/result_agg.csv')
# agg_df.to_json('static/data/result_agg.json',orient="records")
with open('../html/static/data/result.json', 'w+') as f:
    f.write('result(' + point_df.to_json(orient="records") + ')')
with open('../html/static/data/result_agg.json', 'w+') as f:
